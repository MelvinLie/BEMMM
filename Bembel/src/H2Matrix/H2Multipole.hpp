...
struct Moment1D {
  static Eigen::MatrixXd computeMoment1D(const SuperSpace<LinOp> &super_space,
    ...
    int n = 1 << cluster_refinements;      //2^cluster_refinements
    double h = 1. / n;
...
forwardTransformation(const Eigen::MatrixXd &moment_matrices,
  ...
  //std::cout << "long_rhs_fw size = (" << long_rhs_forward.back().rows() << " , " << long_rhs_forward.back().cols() << std::endl;
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    //std::cout << "transferred size = (" << transferred.rows() << " , " << transferred.cols() << ")"<< std::endl;
