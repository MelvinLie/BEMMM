...
#ifndef BEMBEL_ACA_ELEMENTOCTTREE_H_
#define BEMBEL_ACA_ELEMENTOCTTREE_H_
...
class ElementOctTree {
 public:
  ...
  ElementOctTree(AnsatzSpace<Derived> *ansatz_space, unsigned int max_level, const Eigen::Matrix<double,2,3> &bbox) {
    init_ElementOctTree( ansatz_space,max_level, bbox);
  }
  //////////////////////////////////////////////////////////////////////////////
  /// init
  //////////////////////////////////////////////////////////////////////////////
  void init_ElementOctTree(AnsatzSpace<Derived> *ansatz_space,unsigned int max_level, const Eigen::Matrix<double,2,3> &bbox) {

    //ansatz space pointer
    ansatz_space_ = ansatz_space;

    //num_dofs_ = ansatz_space_->get_number_of_dofs();
    num_el_ = ansatz_space_->get_number_of_elements();

    //allocate memory pointer
    mem_ = std::make_shared<ElementOctTreeMemory>();
    //set max level

    mem_->max_level_ = max_level;
    //std::cout << "max level = " << max_level << std::endl;
    //allocate a pointer to the tree nodes
    mem_->memory_ = std::make_shared<std::vector<ElementOctTreeNode>>();
    //allocate memory for the hole tree
    mem_->memory_->resize(mem_->cumNumElements(max_level));

    //std::cout << "cumNumElements = " << mem_->cumNumElements(max_level) << std::endl;
    ...
    ElementOctTreeNode &root = mem_->get_root();
    //initialize the root node element indices
    for (int i = 0; i < num_el_ ; ++i) root.append_index(i);
    ...
    //setup the total element counter
    node_ctr_ = 1;

    //first level starts with root node
    mem_->levels_.push_back(0);
    mem_->levels_.push_back(1);
    ...
    //sort level vector
    std::sort(mem_->levels_.begin(),mem_->levels_.end());
  ...
  */
    ...
    if((*mem_->memory_).size() == 0){
      ...
      std::cout << "Initialize a cluster tree first!" << std::endl;
        ...
        for (int n = 0; n < node_ctr_; ++n){
          ...
          std::cout << "\tNumber of elements = " << tmp_node.indices_.size() << std::endl;
  ...
  std::shared_ptr<ElementOctTreeMemory> get_tree_memory(){
 ...
 private:
  ...
  std::shared_ptr<ElementOctTreeMemory> mem_;
  //int number_of_basis_fcns_;
  int max_level_;
  //number of elements on the ansatz space
  int num_el_;
  //int num_dofs_;
  ...
  */
  ...
  void generate_cluster_tree(int mem_from,int mem_to){
    ...
    for (int n = 0; n < num_root; ++n){
      ...
      if( (lvl_ctr_ < min_tree_level_) || ((*mem_->memory_)[mem_from + n].indices_.size() > min_numel_)){
        ...
        for (int s = 0; s < bs_idx_list.size(); ++s){
          //it can be that there are no elements in this subsection!
          if (bs_idx_list[s].size() == 0) continue;
          else{
            //there are some elements here, this is a node
    ...
    mem_->levels_.push_back(mem_to + son_ctr);
    ...
    std::vector<std::vector<int>> bisect_index_list(Eigen::MatrixXd box, std::vector<int> index_list, std::vector<Eigen::MatrixXd> *bisection){
      ...
      if(rem_indx_list.size() != 0){
        std::cout << "!!!Something went wrong! Not all Elements could be clustered." << std::endl;
    ...
    */
    ...
    Eigen::Vector3d get_cog(int el_indx){

      //get the element tree
      ElementTree el_tree = ansatz_space_->get_superspace().get_mesh().get_element_tree();

      //pointer to first element in lowest level of element tree
      std::vector<ElementTreeNode>::const_iterator element_it = el_tree.cpbegin();

      return element_it[el_indx].midpoint_;
