...
// information.
#ifndef __BEMBEL_DEVICES_MLFMMMATRIXSC_H__
#define __BEMBEL_DEVICES_MLFMMMATRIXSC_H__
...
namespace internal {
template <typename Derived, typename LinOp>
struct traits<MLFMMMatrixSC<Derived,LinOp>> : public internal::traits<SparseMatrix<double>> {};
}  // namespace internal
...
template <typename Derived, typename LinOp>
class MLFMMMatrixSC : public EigenBase<MLFMMMatrixSC<Derived,LinOp>> {
 public:
  //////////////////////////////////////////////////////////////////////////////
  ...
  Product<MLFMMMatrixSC<Derived,LinOp>, Rhs, AliasFreeProduct> operator*(
      const MatrixBase<Rhs>& x) const {
    return Product<MLFMMMatrixSC<Derived,LinOp>, Rhs, AliasFreeProduct>(*this, x.derived());
  }
  //////////////////////////////////////////////////////////////////////////////
  /// constructors
  //////////////////////////////////////////////////////////////////////////////
...
#pragma omp parallel for
      for(std::vector<Bembel::ElementOctTreeNode>::iterator el_it = el_tree_mem->lbegin(l); el_it < el_tree_mem->lend(l) ; ++el_it){
        ...
        if((*el_it).sons_.size() == 0){
          ...
          int_mat.resize(3*num_coeffs,I2_);

          //make space for multipole moment matrix
          (*el_it).moment_mat_.resize(3*num_coeffs,num_DoFs);

          //running index
          //int k = 0;

          //make space for temporal sparse matrix on discontinuous basis
          Eigen::SparseMatrix<std::complex<double>,Eigen::RowMajor> tmp_l(3*num_coeffs,num_DoFs_disc);

          //make tripletList to fill sparse moment matrix
          typedef Eigen::Triplet<std::complex<double>> T;
          std::vector<T> tripletList;
          tripletList.reserve(3*num_coeffs*(*el_it).indices_.size()*I2_);
          ...
          for(int el_indx: (*el_it).indices_){
            ...
            for (auto j = 0; j < Q.w_.size(); ++j) {
              ...
              potential_->evaluateFMMMoments_surface_currents(super_space,mesh_it[el_indx], (*el_it).center_, qp, &int_mat,num_multipoles_);
            ...
            for( int k = 0; k < I2_; ++k){
              for( int m = 0; m < num_coeffs; ++m){
                  ...
                  tripletList.push_back(T(m + num_coeffs , k0 + k , int_mat(m + num_coeffs,k)/4./BEMBEL_PI ));
                  tripletList.push_back(T(m + 2*num_coeffs , k0 + k , int_mat(m + 2*num_coeffs,k)/4./BEMBEL_PI ));
              }
            }
...
#pragma omp parallel for
 ...
 private:
  ...
  // to be private
  MLFMMMatrixSC(const MLFMMMatrixSC<Derived,LinOp>& FMM);
  MLFMMMatrixSC(MLFMMMatrixSC<Derived,LinOp>&& FMM);
  MLFMMMatrixSC& operator=(const MLFMMMatrixSC<Derived,LinOp>& FMM);
  MLFMMMatrixSC& operator=(MLFMMMatrixSC<Derived,LinOp>&& FMM);
