...
// information.
#ifndef __BEMBEL_DEVICES_MLFMMMATRIX_H__
#define __BEMBEL_DEVICES_MLFMMMATRIX_H__
...
namespace internal {
template <typename Derived, typename LinOp>
struct traits<MLFMMMatrix<Derived,LinOp>> : public internal::traits<SparseMatrix<double>> {};
}  // namespace internal
...
template <typename Derived, typename LinOp>
class MLFMMMatrix : public EigenBase<MLFMMMatrix<Derived,LinOp>> {
 public:
  //////////////////////////////////////////////////////////////////////////////
  ...
  Product<MLFMMMatrix<Derived,LinOp>, Rhs, AliasFreeProduct> operator*(
      const MatrixBase<Rhs>& x) const {
    return Product<MLFMMMatrix<Derived,LinOp>, Rhs, AliasFreeProduct>(*this, x.derived());
  }
  //////////////////////////////////////////////////////////////////////////////
  /// constructors
  //////////////////////////////////////////////////////////////////////////////
  ...
  void init_MLFMMMatrix(Bembel::DiscretePotential<Derived,LinOp> *potential,Bembel::MeasurementData *meas_data, int max_tree_lvl, const Eigen::Matrix<double,2,3> &bbox) {//HallProbeArray *device

    potential_ = potential;
    ansatz_space_ = potential->get_ansatz_space();
    Bembel::AnsatzSpace<LinOp> *ansatz_space = potential->get_ansatz_space();
    ...
    double one_over_4_pi = 1./4./BEMBEL_PI;
        ...
        if((*el_it).sons_.size() == 0){
          ...
          for(int el_indx: (*el_it).indices_){
            ...
            for (auto j = 0; j < Q.w_.size(); ++j) {
              ...
              potential->get_potential().evaluateFMMMoments(super_space,mesh_it[el_indx], (*el_it).center_, qp, &int_mat,num_multipoles_);
            ...
            //Save the moments in the tree
            //(*el_it).moment_mat_.block(0,k*polynomial_degree_plus_one_squared,num_coeffs,polynomial_degree_plus_one_squared) = int_mat/4./BEMBEL_PI;
...
#pragma omp parallel for
      for(std::vector<Bembel::MeasurementTreeNode>::iterator meas_it = meas_tree_mem->lbegin(l); meas_it < meas_tree_mem->lend(l) ; meas_it++){
        ...
        if(meas_it->sons_.size() == 0){
          ...
          for(int meas_indx: meas_it->indices_){
            ...
            Eigen::VectorXcd Rlm = Bembel::Rlm_alt(num_multipoles_, d);

            for(int k = 0; k < Rlm.size(); ++ k){
              tripletList.push_back(T(meas_indx , k , Rlm(k) ));
            }

            //meas_it->local_mat_.row(k) = Bembel::Rlm_alt(num_multipoles_, d);
            //++k;
          }
        ...
        for(int source_indx : source_node->indices_){
          ...
          for(int meas_indx : meas_node->indices_){
            ...
            for (auto j = 0; j < Q.w_.size(); ++j) {
              ...
              potential->get_potential().evaluateIntegrand_impl(super_space,mesh_it[source_indx], meas_pos.row(meas_indx).transpose(), qp, &int_mat);
            ...
            for( int k = 0; k < k1-k0; ++k){
              tripletList.push_back(T(meas_indx , k0 + k , int_mat(k,0) ));
            }
          }
  ...
  Eigen::VectorXd evaluate_near_field_transpose(const Eigen::VectorXd &rhs){
    ...
    void test_yourself(){
      ...
      /*
      //we place a cube of this size around the center
      double meas_width = 0.2;

      //we place a cube of this size around the center
      int meas_reso = 5;

      //measurement positions
      Eigen::MatrixXd in_dat = Bembel::read_csv("../../output/thesis/fmm/accelerated/evaluations.csv");
      int num_eval = in_dat.rows();
      Eigen::MatrixXd eval_pos = in_dat.block(0,0,num_eval,3);
      */
 ...
 private:
  ...
  // to be private
  MLFMMMatrix(const MLFMMMatrix<Derived,LinOp>& FMM);
  MLFMMMatrix(MLFMMMatrix<Derived,LinOp>&& FMM);
  MLFMMMatrix& operator=(const MLFMMMatrix<Derived,LinOp>& FMM);
  MLFMMMatrix& operator=(MLFMMMatrix<Derived,LinOp>&& FMM);

  //potential operator
  Bembel::DiscretePotential<Derived,LinOp> *potential_;
        ...
        for(std::vector<Bembel::ElementOctTreeNode>::iterator  el_it = el_tree_mem->lbegin(l); el_it < el_tree_mem->lend(l) ; ++el_it){
          ...
          if(el_it->sons_.size() > 0){
            ...
            for(int i = 0; i < el_it->sons_.size(); ++i){
              ...
              //std::cout << "m_s = " << son->moments_.transpose() << std::endl;
              //std::cout << "m_f = " << el_it->moments_.transpose() << std::endl;
            }
  ...
  Eigen::VectorXd evaluate_far_field(){
      ...
      Eigen::VectorXd result(meas_data_->get_number_of_measurements());
      result.setZero();
      ...
      Eigen::VectorXd my_result(meas_data_->get_number_of_measurements());
      my_result.setZero();
      ...
      for(int i = 0; i < local_leafs_.size(); ++i){
        ...
        //std::cout << "meas_node->locals_ = " << meas_node->locals_ << std::endl;
